 ...
 vec3 nightClear=mix(vec3(.065,.105,.18),vec3(.009,.021,.058),pow(altitude,.6));vec3 nightCloudColor=mix(vec3(.14,.18,.25),vec3(.065,.09,.14),uRain)*(1.-cloud*.35);
 ...
 vec3 ground=plateColor*mix(vec3(1.03,1.02,1.),vec3(1.12,.74,.48),uWarm*.72);ground=mix(ground,vec3(dot(ground,vec3(.2126,.7152,.0722)))*vec3(.74,.82,.89),uRain*.55);vec3 moonGround=vec3(dot(plateColor,vec3(.2126,.7152,.0722)))*vec3(.31,.43,.65);ground=mix(moonGround,ground,uDay);ground+=plateColor*vec3(.28,.062,.018)*uTwilight*sunward*(1.-uRain*.8);vec3 c=mix(ground,skyColor,skyMask);gl_FragColor=vec4(pow(max(c,vec3(0.)),vec3(.4545)),1.);}`});
 ...
 function applySolar(h){const overcast=Math.max(rain,cloud);skyMaterial.uniforms.uRain.value=overcast;const phase=(h-6)/12*Math.PI,elevation=Math.sin(phase),day=smooth(-.15,.42,elevation),direct=smooth(-.01,.20,elevation),warm=1.-smooth(.10,.65,elevation);
 ...
 const moonBlend=1.-smooth(-.16,.04,elevation);const moonDirection=new T.Vector3(-Math.cos(moonPhase),Math.max(.25,moonElevation)*.9,-.65).normalize();uniforms.uSun.value.set(-Math.cos(phase),elevation*.9,-.65).normalize().lerp(moonDirection,moonBlend).normalize();uniforms.uDirect.value=(direct*(1.-moonBlend)+.48*moonBlend)*(1.-overcast*.75);uniforms.uSunTint.value.setRGB(1.08+twilight*.12,.96-warm*.35-twilight*.10,.82-warm*.53-twilight*.06);uniforms.uSunTint.value.lerp(new T.Color().setRGB(.55,.72,1.03),moonBlend);uniforms.uAmbientTint.value.setRGB(.43+day*.57,.55+day*.45,.83+day*.17);uniforms.uFog.value.setRGB(.075+day*.233+twilight*.045,.105+day*.207-twilight*.02,.17+day*.036-twilight*.055);uniforms.uAmbientTint.value.multiply(new T.Color().setRGB(1.-twilight*.08,1.-twilight*.035,1.+twilight*.08));root.dataset.shadowLength=(5.5/Math.max(uniforms.uSun.value.y,.065)).toFixed(2);root.dataset.lightX=uniforms.uSun.value.x.toFixed(4);
 sunlight.position.copy(uniforms.uSun.value).multiplyScalar(80).add(sunlight.target.position);sunlight.color.copy(uniforms.uSunTint.value);sunlight.intensity=3.1*direct*(1.-moonBlend)+.85*moonBlend;ambient.intensity=.65+day*.50;ambient.color.setRGB(.38+day*.24,.49+day*.21,.75+day*.06);
 uniforms.uAmbientTint.value.multiplyScalar(1.-overcast*.24);sunlight.intensity*=1.-overcast*.87;ambient.intensity*=1.-overcast*.24;
...
float treeOcclusion(vec3 p){vec3 l=normalize(uSun);float height=6.5-p.y;if(height<=0.)return 0.;float travel=height/max(l.y,.065);vec2 hit=(p+l*travel).xz-vec2(20.,-34.);float softness=1.+travel*.012;float crown=exp(-dot(hit/vec2(3.2*softness,2.8*softness),hit/vec2(3.2*softness,2.8*softness))*1.6);vec2 lobe=hit-vec2(2.0,.5);crown=max(crown,exp(-dot(lobe/(2.0*softness),lobe/(2.0*softness))*1.7)*.75);lobe=hit+vec2(1.8,.9);crown=max(crown,exp(-dot(lobe/(2.1*softness),lobe/(2.1*softness))*1.5)*.85);return crown*(1.-smoothstep(85.,140.,travel));}
 ...
 clockInput.addEventListener('input',()=>{hourFrom=hour;targetHour=Number(clockInput.value);hourChanged=performance.now();clockLabel();dirty=true;wake();},{signal:lifecycle.signal});
