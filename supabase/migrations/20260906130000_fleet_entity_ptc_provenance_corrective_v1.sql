-- Corrective migration: the original fleet RPC may already be applied, so this
-- replacement adds trusted PTC provenance without rewriting migration history.
  ...
  if p_kind = 'vehicle' then
    ...
    begin
      insert into public.reference_vehicles(
        ...
        plate_number, license_plate, status, is_active, archived, import_source
      ) values (
        p_company, p_actor, trim(p_name), trim(p_name), trim(p_name), 'truck', 'truck',
        trim(p_plate), trim(p_plate), 'free', true, false, 'ptc_fleet_manager_manual_v1'
