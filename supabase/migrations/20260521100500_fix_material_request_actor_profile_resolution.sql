...
begin
  ...
  where (p.id = p_actor_user_id or p.user_id = p_actor_user_id or p.id = v_auth_uid or p.user_id = v_auth_uid)
    and (p.id = v_auth_uid or p.user_id = v_auth_uid)
  order by
    case
      when p.id = p_actor_user_id then 0
      when p.user_id = p_actor_user_id then 1
      when p.id = v_auth_uid then 2
      else 3
    end
  limit 1;
