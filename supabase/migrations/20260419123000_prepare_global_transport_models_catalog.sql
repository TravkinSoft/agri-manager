...
begin
  ...
  if lower(new.brand) = lower(new.model) then
    raise exception 'model must not duplicate brand';
  end if;

  new.full_name := trim(
    concat_ws(
      ' ',
      nullif(trim(new.brand), ''),
      nullif(trim(new.series), ''),
      nullif(trim(new.model), '')
    )
  );

  if new.full_name = '' then
    raise exception 'full_name is empty after normalization';
  end if;
