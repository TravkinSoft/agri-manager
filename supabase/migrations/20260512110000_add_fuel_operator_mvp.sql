...
-- Canonical production issue implementation.
CREATE OR REPLACE FUNCTION public.issue_fuel_mvp(p_company_id uuid, p_actor_user_id uuid, p_fuel_source_id uuid, p_vehicle_id uuid, p_mechanizator_id uuid DEFAULT NULL::uuid, p_liters numeric DEFAULT 0, p_issued_at timestamp with time zone DEFAULT now(), p_comment text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  select * into v_actor from public.profiles where id = p_actor_user_id limit 1;
  if not found then raise exception 'Actor profile not found'; end if;
  if v_actor.company_id <> p_company_id then raise exception 'Actor does not belong to company'; end if;
  if coalesce(v_actor.status,'active') <> 'active' then raise exception 'Actor profile is not active'; end if;
  if coalesce(v_actor.role,'') not in ('global_admin','company_admin','warehouse','fuel_operator') then
    raise exception 'Actor role cannot issue fuel';
  end if;
  ...
  select * into v_source
  from public.fuel_sources
  where id = p_fuel_source_id and company_id = p_company_id
  for update;
  if not found then raise exception 'Fuel source not found'; end if;
  if v_source.archived or not v_source.is_active then raise exception 'Fuel source is not active'; end if;
  if v_source.current_balance_liters < p_liters then
    raise exception 'Insufficient fuel balance. Available %, requested %', v_source.current_balance_liters, p_liters;
  end if;
  ...
  limit 1;
  if not found then raise exception 'Vehicle not found in company'; end if;
  if coalesce(v_vehicle.archived,false) then raise exception 'Vehicle is archived'; end if;

  if p_mechanizator_id is not null then
    select * into v_specialist
    from public.reference_specialists
    where id = p_mechanizator_id and company_id = p_company_id
    limit 1;
    if not found then raise exception 'Mechanizator/responsible person not found in company'; end if;
    if coalesce(v_specialist.archived,false) then raise exception 'Mechanizator/responsible person is archived'; end if;
  end if;

  insert into public.fuel_issues(
    company_id, issued_at, fuel_source_id, fuel_type, vehicle_id, mechanizator_id, liters, comment, created_by_user_id
  ) values (
    p_company_id, coalesce(p_issued_at,now()), p_fuel_source_id, v_source.fuel_type, p_vehicle_id, p_mechanizator_id,
    p_liters, nullif(trim(coalesce(p_comment,'')),''), p_actor_user_id
  )
  returning id into v_issue_id;

  update public.fuel_sources
  set current_balance_liters = current_balance_liters - p_liters,
      updated_by_user_id = p_actor_user_id,
      updated_at = now()
  where id = p_fuel_source_id;
