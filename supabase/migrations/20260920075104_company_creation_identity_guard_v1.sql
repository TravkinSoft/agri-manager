-- Archive orphan tenants without cascading into immutable warehouse ledgers.
alter table public.companies add column if not exists archived_at timestamptz;

alter table public.companies add constraint companies_name_nonblank_v1
  check (length(btrim(regexp_replace(replace(name, chr(160), ' '), '[[:space:]]+', ' ', 'g'))) > 0);

-- Preserve trusted invitation markers, roles, pending state, grants and definer search_path.
