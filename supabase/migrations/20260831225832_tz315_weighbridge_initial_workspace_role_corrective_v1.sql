...
do $$
begin
  if pg_catalog.to_regprocedure('public.weighbridge_initial_workspace_v1(uuid,text,boolean)') is null then
    raise exception 'TZ315 requires public.weighbridge_initial_workspace_v1(uuid,text,boolean)';
  end if;
end
$$;

-- Corrective narrowing: preserve the original HTTP route role matrix exactly.
