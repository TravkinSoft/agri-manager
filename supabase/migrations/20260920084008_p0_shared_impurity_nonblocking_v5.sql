...
-- Closed documents are untouched. Existing open documents use the same rules.
-- Function security mode and grants are preserved, not expanded.
do $guard$
begin
  if pg_catalog.md5(pg_catalog.pg_get_functiondef(
    'public.create_weighbridge_shared_impurity_pool_ticket_v1(uuid,uuid,jsonb,uuid,uuid,numeric,text,text,text,uuid)'::regprocedure
  )) <> 'ec14ebed239262dd1bc20ce64b0dfe0b'
  ...
  )) <> 'e0caf75e2ace553562027945a31a9e4f'
  then raise exception 'SHARED_IMPURITY_V5_BASELINE_CHANGED'; end if;
end
$guard$;

drop index public.weighbridge_shared_impurity_source_batches_active_batch_uq;
...
where state = 'reclassified';

create or replace view public.v_weighbridge_open_ticket_reservations_v1
with (security_invoker = true) as
select t.company_id, t.id as ticket_id, t.warehouse_from_id as warehouse_id,
  case when coalesce(tl.batch_id, '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    then tl.batch_id::uuid else null end as batch_id,
  pg_catalog.sum(coalesce(tl.quantity_kg, tl.net_line_weight_kg, tl.mass_kg, tl.quantity, 0))::numeric(18,6) as reserved_kg
from public.tickets t
join public.ticket_lines tl on tl.ticket_id = t.id and tl.company_id = t.company_id
...
group by t.company_id,t.id,t.warehouse_from_id,
  case when coalesce(tl.batch_id, '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
    then tl.batch_id::uuid else null end;
...
begin
  ...
  loop
    ...
    if exists (
      ...
      where existing_source.inventory_batch_id = v_source.id
        and existing_source.state = 'reclassified'
    ) then
  ...
  if v_member_count <> pg_catalog.cardinality(v_ids)
     ...
     or exists (
       select 1 from public.v_weighbridge_open_ticket_reservations_v1 reservation
...
declare
  ...
  v_scope_hash text;
  v_live_source_total numeric(18,6) := 0;
  v_live_source_weights jsonb := '{}'::jsonb;
  v_source_scope jsonb;
  v_expires timestamptz := pg_catalog.now() + interval '24 hours';
begin
  if v_principal_id is null then
  ...
  loop
    ...
    v_live_source_total := v_live_source_total + v_current_balance;
    v_live_source_weights := v_live_source_weights
      || pg_catalog.jsonb_build_object(v_source.inventory_batch_id::text, v_current_balance);
  end loop;
  if v_source_count < 1
  ...
  if v_member_count < 1
     ...
     -- Open impurity documents select provenance, not the whole stock balance.
     or exists (
       select 1 from public.v_weighbridge_open_ticket_reservations_v1 reservation
  ...
  -- V5: identity and original snapshot hashes have passed the unchanged checks
  -- above. Re-read mass under the company/season gate and sorted batch locks.
  -- This is the only point where selected mass becomes committed. Another
  -- open impurity ticket is not a stock promise; ordinary reservations and
  -- processing allocations are still rejected above.
  v_clean := pg_catalog.round(v_live_source_total - v_impurity, 6);
  if v_clean < 0 then
    raise exception 'IMPURITY_WEIGHT_EXCEEDS_AVAILABLE|%', v_live_source_total
      using errcode = '23514';
  end if;
  if pg_catalog.abs(v_live_source_total - v_pool.source_total_kg) > 0.000001
     or exists (
       select 1 from public.weighbridge_shared_impurity_source_batches s
       where s.group_id = v_pool.id
         and s.source_balance_snapshot_kg <> (v_live_source_weights ->> s.inventory_batch_id::text)::numeric
     )
  then
    update public.weighbridge_shared_impurity_source_batches s
    set source_balance_snapshot_kg = (v_live_source_weights ->> s.inventory_batch_id::text)::numeric
    where s.group_id = v_pool.id;

    update public.weighbridge_shared_impurity_members m
    set source_total_snapshot_kg = (
      select pg_catalog.sum(s.source_balance_snapshot_kg)
      from public.weighbridge_shared_impurity_source_batches s
      where s.group_id = m.group_id and s.member_id = m.id
    ), updated_at = pg_catalog.now()
    where m.group_id = v_pool.id;

    update public.weighbridge_shared_impurity_groups g
    set audit_json = g.audit_json || pg_catalog.jsonb_build_object(
          'selection_rebased_at_close_v5', true,
          'opening_source_total_kg', v_pool.source_total_kg,
          'opening_source_scope_hash', v_pool.source_scope_hash,
          'opening_composition_snapshot', v_pool.composition_snapshot,
          'opening_composition_hash', v_pool.composition_hash
        ),
        source_total_kg = v_live_source_total,
        composition_snapshot = (
          select pg_catalog.jsonb_agg(
            element.value || pg_catalog.jsonb_build_object(
              'pre_impurity_source_mass_kg',
              (v_live_source_weights ->> (element.value ->> 'inventory_batch_id'))::numeric
            ) order by element.ordinality
          )
          from pg_catalog.jsonb_array_elements(g.composition_snapshot)
        ...
        source_scope_hash = (
          select pg_catalog.encode(extensions.digest(pg_catalog.convert_to(
            pg_catalog.jsonb_agg(pg_catalog.jsonb_build_object(
              ...
              'inventory_batch_id', s.inventory_batch_id,
              'source_balance_snapshot_kg', s.source_balance_snapshot_kg
            ) order by s.inventory_batch_id)::text, 'UTF8'), 'sha256'), 'hex')
          from public.weighbridge_shared_impurity_source_batches s
          where s.group_id = g.id
        ),
        updated_at = pg_catalog.now()
    where g.id = v_pool.id;

    update public.weighbridge_shared_impurity_groups g
    set composition_hash = pg_catalog.encode(extensions.digest(
      pg_catalog.convert_to(g.composition_snapshot::text, 'UTF8'), 'sha256'), 'hex')
    where g.id = v_pool.id;
    select g.* into v_pool from public.weighbridge_shared_impurity_groups g
    where g.id = v_pool.id;
  end if;
  v_source_sum := v_live_source_total;
  v_member_sum := v_live_source_total;
  v_scope_hash := v_pool.source_scope_hash;
...
comment on view public.v_weighbridge_open_ticket_reservations_v1 is
  'Actual ordinary ticket commitments. Open shared impurity source snapshots are not reservations; exact stock is locked and checked atomically at close (V5).';
-- Source counts vary by orders of magnitude. Do not reuse a plan learned from
-- tiny earlier tickets for a full party; include the settlement wrapper too.
alter function public.finalize_weighbridge_shared_impurity_pool_ticket_v1(uuid,text,numeric,boolean,uuid)
