-- Add two restricted company-account roles. Existing roles and personnel records stay intact.
...
alter table public.profiles add constraint valid_role check (role = any(array[
  ...
  'mechanic_operator','vegetable_brigadier'
]::text[]));

-- Called only by the authenticated administrator's server invite endpoint.
-- Profile/person binding is one transaction, never an inferred match by name.
create function public.ptc_bind_invited_profile_v1(
...
begin
  if p_role not in ('mechanic_operator','vegetable_brigadier') or length(trim(p_name))=0 then
  ...
  -- Serialize same-company bindings, including the previously unlinked employee case.
  ...
  if result_person is null then
    insert into public.company_people(company_id,user_id,full_name,role_type,position,status,created_by_user_id,updated_by_user_id)
      values(p_company,p_user,trim(p_name),case when p_role='mechanic_operator' then 'mechanic_operator' else 'manager' end,
        case when p_role='mechanic_operator' then 'Механизатор' else 'Бригадир овощной' end,'active',p_actor,p_actor)
