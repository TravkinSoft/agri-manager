  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-3 lg:grid-cols-[minmax(220px,1fr)_200px_200px_160px_160px]">
        <div className="relative"><Search className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Материал, пользователь, документ" /></div>
      ...
      <div className="overflow-x-auto border-y border-border">
        <Table><TableHeader><TableRow><TableHead>Дата и время</TableHead><TableHead>Материал</TableHead><TableHead>Направление</TableHead><TableHead className="text-right">Количество</TableHead><TableHead>Склад</TableHead><TableHead>Основание</TableHead><TableHead>Пользователь</TableHead></TableRow></TableHeader>
          <TableBody>{loading ? <TableRow><TableCell colSpan={7} className="py-10 text-center text-muted-foreground">Загрузка...</TableCell></TableRow> : filtered.length === 0 ? <TableRow><TableCell colSpan={7} className="py-10 text-center text-muted-foreground">Движения не найдены</TableCell></TableRow> : filtered.map((row) => {
            const delta = Number(row.quantity_delta ?? (row.transaction_type === "in" ? row.quantity : -row.quantity));
            return <TableRow key={row.id} className="cursor-pointer hover:bg-background" onClick={() => setSelectedId(row.id)}><TableCell>{formatDate(row.operation_datetime || row.created_at)}</TableCell><TableCell className="font-medium">{row.product_name}</TableCell><TableCell><Badge variant="outline">{delta >= 0 ? "IN" : "OUT"}</Badge></TableCell><TableCell className={`text-right font-medium ${delta >= 0 ? "text-emerald-800" : "text-red-800"}`}>{delta >= 0 ? "+" : ""}{delta.toLocaleString("ru-RU")} {localizeUnit(row.product_unit || "", language)}</TableCell><TableCell>{row.warehouse_name}</TableCell><TableCell>{row.document_ref || row.reason_type || "-"}</TableCell><TableCell>{row.created_by_email || "-"}</TableCell></TableRow>;
          })}</TableBody>
        </Table>
      </div>

      <Dialog open={Boolean(selected)} onOpenChange={(open) => !open && setSelectedId(null)}>
        <DialogContent className="sm:max-w-2xl">{selected ? <><DialogHeader><DialogTitle className="flex items-center gap-2"><ArrowRightLeft className="h-5 w-5 text-amber-800" />{movementLabel(selected)}</DialogTitle><DialogDescription>{formatDate(selected.operation_datetime || selected.created_at)}</DialogDescription></DialogHeader><div className="grid gap-4 text-sm sm:grid-cols-2"><div><div className="text-muted-foreground">Материал</div><div className="mt-1 font-medium">{selected.product_name}</div></div><div><div className="text-muted-foreground">Количество</div><div className="mt-1 font-medium">{Number(selected.quantity_delta || selected.quantity).toLocaleString("ru-RU")} {localizeUnit(selected.product_unit || "", language)}</div></div><div><div className="text-muted-foreground">Склад</div><div className="mt-1">{selected.warehouse_name}</div></div><div><div className="text-muted-foreground">Пользователь</div><div className="mt-1">{selected.created_by_email || "-"}</div></div><div><div className="text-muted-foreground">Документ / заявка</div><div className="mt-1">{selected.document_ref || selected.reason_ref_id || "-"}</div></div><div><div className="text-muted-foreground">Источник</div><div className="mt-1">{selected.reason_type || selected.source_system || "-"}</div></div></div></> : null}</DialogContent>
      </Dialog>
    </div>
  );
}
