...
export default function WarehouseInventoryPage() {
  ...
  return <div className="space-y-5">
    ...
    {loading ? <div className="py-10 text-center text-sm text-muted-foreground">Загрузка...</div> : selected ? <section className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3 border-y border-border py-3">
        <div><h2 className="font-semibold">{selected.inventory_no} · {selected.warehouse_name}</h2><div className="text-sm text-muted-foreground">Начата {formatDate(selected.started_at)} · ответственный: {selected.assigned_to_name || "Не назначен"}</div></div>
      ...
      <div className="overflow-x-auto"><Table><TableHeader><TableRow><TableHead>Материал</TableHead><TableHead className="text-right">По учёту</TableHead><TableHead className="text-right">Фактически</TableHead><TableHead className="text-right">Разница</TableHead></TableRow></TableHeader><TableBody>{(selected.items || []).map((item) => {
        ...
        return <TableRow key={item.id}><TableCell><span className="font-medium">{item.product_name_snapshot}</span>{item.batch_id_text ? <div className="text-xs text-muted-foreground">Партия: {item.batch_id_text}</div> : null}</TableCell><TableCell className="text-right">{quantity(item.book_quantity)} {item.uom}</TableCell><TableCell className="text-right">{editable ? <div className="ml-auto flex w-40 items-center gap-2"><Input className="text-right" type="number" min="0" step="0.001" value={actualValue ?? ""} onChange={(event) => setActual((current) => ({ ...current, [item.id]: event.target.value }))} /><span>{item.uom}</span></div> : `${quantity(item.actual_quantity)} ${item.uom}`}</TableCell><TableCell className={cn("text-right font-medium", number(difference) > 0 ? "text-emerald-800" : number(difference) < 0 ? "text-red-800" : "text-muted-foreground")}>{actualValue === "" && difference == null ? "—" : `${number(difference) > 0 ? "+" : ""}${quantity(difference)} ${item.uom}`}{number(difference) !== 0 ? <div className="text-xs font-normal text-muted-foreground">Предполагаемый ledger {number(difference) > 0 ? "IN" : "OUT"}</div> : null}</TableCell></TableRow>;
      })}</TableBody></Table></div>
      ...
      {["approved","cancelled"].includes(selected.status) ? <div className="text-sm text-muted-foreground">{selected.status === "approved" ? `Подтверждена ${formatDate(selected.approved_at)}` : "Отменена"} · проверено {selected.item_count} позиций · расхождений {selected.difference_count}.</div> : null}
    </section> : <div className="py-10 text-center text-sm text-muted-foreground">Инвентаризаций пока нет.</div>}

    <section><h2 className="mb-3 text-lg font-semibold">История</h2><div className="space-y-2">{inventories.map((row) => <button key={row.id} type="button" onClick={() => setSelectedInventoryId(row.id)} className={cn("grid w-full gap-1 border-y border-border px-2 py-3 text-left text-sm hover:bg-background sm:grid-cols-[180px_1fr_170px_140px]", row.id === selectedInventoryId && "bg-background")}><span className="font-medium">{row.inventory_no}</span><span>{row.warehouse_name}</span><span>{formatDate(row.started_at)}</span><span>{statusLabel(row.status)} · {row.difference_count} расх.</span></button>)}</div></section>
  </div>;
}
