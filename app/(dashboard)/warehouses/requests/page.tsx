...
export default function WarehouseRequestsPage() {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    try {
      const [requestRows, warehouseRows, balanceRows] = await Promise.all([
    ...
    setPreparedByItem(
      Object.fromEntries(
        (selectedRequest.items || []).map((item) => [
          item.id,
          numberText(toQty(item.prepared_quantity, 0)),
        ])
      )
    );
    setReturnByItem(
  ...
  const stockRows = useMemo(() => {
    ...
    return (selectedRequest.items || []).map((item) => {
      ...
      const prepared = Math.max(
        toQty(preparedByItem[item.id], toQty(item.prepared_quantity, 0)),
        0
      );
      const issued = toQty(item.issued_quantity, 0);
      const available = effectiveWarehouseId
      ...
      return {
        ...
        issued,
        available,
        remainingToPrepare: Math.max(planned - prepared, 0),
        deviation: prepared - planned,
        remainingToIssue: Math.max(prepared - issued, 0),
        exceedsStock: prepared > available + 0.000001,
      };
    });
  }, [
    balances,
  ...
  const handleReady = async () => {
    ...
    if (hasStockProblem) {
      toast({
        ...
        description: "Подготовленное количество не может превышать остаток.",
        variant: "destructive",
      });
      return;
    }
      ...
      () =>
        updateWarehouseIssueRequestStatus({
          ...
          items: stockRows.map((row) => ({
            itemId: row.item.id,
            preparedQuantity: Number(row.prepared.toFixed(4)),
          })),
        }),
            ...
            <>
              ...
              <div className="min-h-0 space-y-7 overflow-y-auto px-4 py-5 sm:px-6">
                ...
                <section className="space-y-3">
                  ...
                  <div className="hidden grid-cols-[minmax(170px,1.4fr)_100px_repeat(5,minmax(86px,0.7fr))] gap-3 border-b border-slate-800 pb-2 text-[12px] text-slate-500 md:grid">
                    <span>Материал</span>
                    <span>Категория</span>
                    <span>План</span>
                    <span>Доступно</span>
                  ...
                  <div className="space-y-2 md:space-y-0">
                    {stockRows.map((row) => (
                      <div
                        key={row.item.id}
                        className="grid gap-3 rounded-lg border border-slate-800 p-3 text-sm md:grid-cols-[minmax(170px,1.4fr)_100px_repeat(5,minmax(86px,0.7fr))] md:items-center md:rounded-none md:border-x-0 md:border-t-0 md:px-0 md:py-3"
                      >
                        <div className="min-w-0">
                          ...
                          {preparing && canProcess ? (
                            <div className="relative max-w-36">
                              <Input
                                ...
                                max={row.available}
                                step="0.01"
                                value={preparedByItem[row.item.id] ?? "0"}
                                onChange={(event) =>
                        ...
                        <div>
                          ...
                          {numberText(row.remainingToPrepare)} {row.unit}
                        </div>
                        <div
                          className={
                            Math.abs(row.deviation) < 0.000001
                              ? "text-emerald-300"
                              : "text-amber-300"
                          }
                        >
                          ...
                          {row.deviation > 0 ? "+" : ""}
                          {numberText(row.deviation)} {row.unit}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {hasStockProblem ? (
                  <Alert variant="destructive">
                    <AlertDescription>
                      Подготовленное количество превышает фактический остаток на
                      выбранном складе.
                    </AlertDescription>
                  </Alert>
                ) : null}
                ...
                {preparing && canProcess ? (
                  <div className="flex justify-end">
                    <Button
                      ...
                      disabled={submitting || hasStockProblem || !hasPrepared}
                    >
