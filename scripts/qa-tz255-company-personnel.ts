...
check("driver cannot be selected for tractor", () => assert.equal(personnelRoleMatchesVehicle("driver", { type: "tractor" }), false));
...
check("ticket validates vehicle compatibility", () => assert.match(tickets, /personnelRoleMatchesVehicle/));
check("UI keeps FIO search", () => assert.match(page, /driverSearch/));
