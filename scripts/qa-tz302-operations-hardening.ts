...
async function main() {
  ...
  await check("historical paper contract remains backend-only on the weighbridge form", () => {
    assert.doesNotMatch(weighbridge, /Исторический талон из бумажного журнала/);
    assert.doesNotMatch(weighbridge, /Номер бумажного талона/);
    assert.doesNotMatch(weighbridge, /type="datetime-local"/);
    assert.doesNotMatch(weighbridge, /Бумажная тара, кг/);
