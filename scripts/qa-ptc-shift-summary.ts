...
assert.match(route, /const \{ companyId \} = await dashboardSummaryReader\(request\)/); checks += 1;
...
assert.match(dashboardSummaryReaderHelper, /getServerActorFromSession\(request, \{\s*skipCache: true,\s*\}\)/); checks += 1;
assert.doesNotMatch(dashboardSummaryReaderHelper, /ignoreImpersonation:\s*true/); checks += 1;
assert.match(dashboardSummaryReaderHelper, /\["agronomist", "director"\]\.includes\(actor\.role\)/); checks += 1;
assert.match(dashboardSummaryReaderHelper, /actorUserId: actor\.id/); checks += 1;
assert.match(dashboardSummaryReaderHelper, /allowedRoles: \["agronomist", "director"\]/); checks += 1;
...
assert.match(server, /const MAX_SHIFT_EVENT_ROWS = 10_000/); checks += 1;
assert.match(server, /created_at\.gt\.\$\{cursor\.createdAt\},and\(created_at\.eq\.\$\{cursor\.createdAt\},id\.gt\.\$\{cursor\.id\}\)/); checks += 1;
assert.match(server, /query\.limit\(pageSize \+ 1\)/); checks += 1;
assert.match(server, /throw new TrafficShiftReconstructionLimitError/); checks += 1;
assert.doesNotMatch(server, /\.range\(/); checks += 1;
...
assert.ok(dashboard.indexOf("Главные показатели картофеля") < dashboard.indexOf("<TrafficShiftSummary")); checks += 1;
assert.match(dashboard, /Отчёт PTC по закрытой смене[\s\S]*?shiftReportOpen \? <div[\s\S]*?<TrafficShiftSummary/); checks += 1;
