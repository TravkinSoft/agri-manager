...
export async function readVehicleRepairs(db: SupabaseClient, companyId: string, vehicleIds: string[]) {
  const result = new Map<string, { inRepair: boolean; repairVersion: number; changedAt: string | null }>();
  for (let from = 0; from < vehicleIds.length; from += 250) {
    const rows = await db.from("fleet_vehicle_repairs").select("vehicle_id,in_repair,version,changed_at")
    ...
    for (const row of rows.data ?? []) result.set(row.vehicle_id, {
      inRepair: row.in_repair, repairVersion: row.version, changedAt: row.changed_at,
    });
  }
  return result;
}
