...
export function isIndependentFieldSearchRequest(message: string): boolean {
  const text = normalizeScopeText(message);
  if (!/(?:^|\s)(?:пол(?:е|я|ей)|field)\p{L}*(?=$|\s)/u.test(text)) return false;
  if (hasExplicitFieldReferenceText(text)) return false;
  return hasExplicitFieldSearchQualifier(text);
}
...
export function scopeThreadStateForMessage(
  ...
  if (
    ...
    !isIndependentFieldSearchRequest(message) &&
