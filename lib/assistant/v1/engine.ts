...
function requiredCurrentDataToolCall(params: {
  ...
  if (isIndependentFieldSearchRequest(text)) {
    return { id: `readonly-required-field-search-${params.turn}`, type: "function", function: { name: "search_fields", arguments: "{}" } };
  }
...
function adaptRequestedToolCalls(message: string, calls: any[]): any[] {
  ...
  if ((isGenericFieldList || isIndependentFieldSearchRequest(message)) && calls.length) {
...
export async function runReadOnlyAssistantV1(params: {
  ...
  for (let turn = 0; turn < MAX_MODEL_TURNS; turn += 1) {
    ...
    if (requestedToolCalls.length && toolCalls.length > 0) {
      const previous = toolCalls[toolCalls.length - 1];
      requestedToolCalls = requestedToolCalls.filter((call: any) => {
        const rawName = clean(call?.function?.name) || "unknown";
        if (!isReadOnlyModelToolName(rawName) || rawName !== previous.tool) return true;
        const candidateArgs = normalizedToolArgs({
          name: rawName,
          rawArgs: safeArgs(call?.function?.arguments),
          message,
          state: nextState,
          runtimeContext,
        });
        return JSON.stringify(candidateArgs) !== JSON.stringify(previous.params || {});
      });
    }
    if (!requestedToolCalls.length && modelToolsEnabled && toolCalls.length > 0) {
      ...
      const shouldRetryEmptyFieldDirectory = !answer && isGenericFieldDirectoryRequest(message);
      if (
        (scopeMismatch || shouldRetryEmptyFieldDirectory) &&
