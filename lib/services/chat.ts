...
export async function getChats(userId: string, companyId?: string): Promise<Chat[]> {
  let query = supabase
    .from('chats')
    .select('*')
    .eq('user_id', userId)
...
export async function createChat(userId: string, companyId: string, title: string = 'New Chat'): Promise<Chat> {
  if (!companyId) {
