...
export function buildWarehouseFieldOrigins(tickets: OriginTicket[], areas: OriginArea[], names: Map<string, string>, yieldTickets: OriginTicket[] = tickets): WarehouseFieldOrigin[] {
  ...
  for (const ticket of Array.from(new Map(yieldTickets.map(row => [row.id, row])).values())) {
    ...
    if (plot && plot.field_id === ticket.field_id && Number.isFinite(mass) && mass > 0) yieldMassByPlot.set(plot.id, (yieldMassByPlot.get(plot.id) || 0) + mass);
  }
  ...
  for (const ticket of Array.from(new Map(tickets.map(row => [row.id, row])).values())) {
    ...
    const group = groups.get(key) || { row: { fieldId: ticket.field_id, fieldName: names.get(key) || "Поле не уточнено", netWeightKg: 0, tripCount: 0, areaHa: null, yieldTPerHa: null }, plots: new Set<string>(), missingArea: false };
  ...
  return Array.from(groups.values()).map(({ row, plots, missingArea }) => {
    ...
    return { ...row, areaHa: !missingArea && area > 0 ? area : null, yieldTPerHa: !missingArea && area > 0 ? yieldMass / 1000 / area : null };
