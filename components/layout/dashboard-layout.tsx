      ...
      <AssistantShellProvider>
        <div className={`travkin-shell tf-manor-shell flex ${isTraffic || isDashboard ? "h-[100dvh] min-h-0 overflow-hidden md:h-screen" : "min-h-screen"}`}>
          ...
          <div className="flex min-h-0 min-w-0 flex-1 flex-col">
            <Header />
            <main className={`travkin-scrollbar tf-manor-workspace flex min-h-0 flex-1 flex-col overflow-x-hidden overflow-y-scroll [scrollbar-gutter:stable] pl-[max(0.75rem,env(safe-area-inset-left))] pr-[max(0.75rem,env(safe-area-inset-right))] sm:pl-[max(1rem,env(safe-area-inset-left))] sm:pr-[max(1rem,env(safe-area-inset-right))] md:pl-[max(1.5rem,env(safe-area-inset-left))] md:pr-[max(1.5rem,env(safe-area-inset-right))] ${isDashboard || isWarehouses ? "pt-1.5 sm:pt-2 md:pt-3" : "py-3 sm:py-4 md:py-6"} ${isWarehouses ? "pb-[calc(env(safe-area-inset-bottom)+6.25rem)] md:pb-0" : profile?.role === "fleet_manager" ? "pb-[calc(env(safe-area-inset-bottom)+1rem)]" : "pb-[calc(env(safe-area-inset-bottom)+6.25rem)] sm:pb-[calc(env(safe-area-inset-bottom)+6.25rem)] md:pb-6"}`}>
