...
export function DailyReconciliation({
  ...
  if (loading) return <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Загрузка сверки…</div>;

  return (
    <section aria-label="Сверка бумажного журнала" className="space-y-2">
      <div className="flex items-end justify-between gap-3">
        <div>
          <div className="text-xs font-semibold uppercase text-muted-foreground">Сверка бумажного журнала</div>
          <p className="mt-1 text-xs text-muted-foreground">Контрольный итог не создаёт талон, партию, остаток или проводку.</p>
        </div>
        {error ? <Button type="button" variant="ghost" size="sm" onClick={() => void loadControls()}>Повторить</Button> : null}
      </div>
      {error ? <div role="alert" className="rounded-md border border-red-500/30 bg-red-500/10 px-3 py-2 text-xs text-red-800">{error}</div> : null}
        ...
        <table className="w-full min-w-[980px] text-left text-xs">
          ...
          <tbody className="divide-y divide-border text-foreground">
            {rows.map((row) => {
              ...
              return [
                <tr key={key} className={row.fieldId ? "bg-background" : "bg-background font-semibold"}>
                  ...
                  <td className="px-3 py-2"><Input aria-label={`Бумажный итог ${row.fieldName} ${row.day}`} inputMode="decimal" value={drafts[key] ?? (paper == null ? "" : String(paper))} onChange={(event) => setDrafts((current) => ({ ...current, [key]: event.target.value }))} className="h-8 w-32 border-border bg-background" placeholder="кг" /></td>
                  <td className={`px-3 py-2 ${differenceKg == null ? "text-muted-foreground" : Math.abs(differenceKg) < 0.001 ? "text-emerald-800" : "text-amber-800"}`}>{differenceKg == null ? "Не задан" : differenceKg === 0 ? "Сходится" : kg(differenceKg)}</td>
                  <td className="px-2 py-2"><div className="flex gap-1"><button type="button" title="Сохранить контрольный итог" aria-label="Сохранить контрольный итог" onClick={() => void save(row)} disabled={saving === key} className="grid h-8 w-8 place-items-center rounded-md border border-border hover:bg-muted disabled:opacity-50">{saving === key ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}</button>{row.aggregate.ticketIds?.length ? <button type="button" title="Показать рейсы" aria-label="Показать рейсы" onClick={() => setOpenRows((current) => ({ ...current, [key]: !expanded }))} className="grid h-8 w-8 place-items-center rounded-md border border-border hover:bg-muted"><ChevronDown className={`h-4 w-4 transition-transform ${expanded ? "rotate-180" : ""}`} /></button> : null}</div></td>
                </tr>,
                expanded ? <tr key={`${key}:tickets`}><td colSpan={9} className="bg-background px-4 py-2"><div className="flex flex-wrap gap-2">{(row.aggregate.ticketIds || []).map((ticketId, index) => <button key={ticketId} type="button" onClick={() => onOpenTicket(ticketId)} className="inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs text-foreground hover:border-yellow-500/60 hover:text-foreground">Рейс {index + 1}<ExternalLink className="h-3 w-3" /></button>)}</div></td></tr> : null,
              ];
            })}
            {rows.length === 0 ? <tr><td className="px-3 py-4 text-muted-foreground" colSpan={9}>Завершённых рейсов пока нет.</td></tr> : null}
          </tbody>
        </table>
      </div>
    </section>
