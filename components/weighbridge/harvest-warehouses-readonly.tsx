...
export function HarvestWarehousesReadonly({ companyId }: { companyId: string }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {loading ? <div className="py-12 text-center text-sm text-muted-foreground">Загрузка партий урожая...</div> : null}
      {!loading && !error && groups.length === 0 ? (
        <div className="border-y border-border py-12 text-center text-sm text-muted-foreground">Принятые партии урожая не найдены.</div>
      ) : null}
        ...
        {groups.map(([warehouseId, group]) => (
          <section key={warehouseId} className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <Badge className="bg-muted text-foreground">{group.batches.length}</Badge>
            </div>
            <div className="grid gap-3 xl:grid-cols-2">
              {group.batches.map((batch) => (
                <Card key={batch.id} className="rounded-md border-border bg-background">
                  <CardHeader className="pb-3">
                    <CardTitle className="flex items-start justify-between gap-3 text-base">
                      <span className="flex min-w-0 items-center gap-2"><PackageOpen className="h-4 w-4 shrink-0 text-emerald-800" /><span className="truncate">{[batch.cropName, batch.varietyName].filter(Boolean).join(" / ")}</span></span>
                      <Badge variant="outline" className="shrink-0">{batch.batchCode}</Badge>
                    </CardTitle>
                    <div className="text-sm text-muted-foreground">{[batch.fieldName, batch.reproductionName].filter(Boolean).join(" · ")}</div>
                  </CardHeader>
                  <CardContent className="space-y-3">
                    <div className="grid grid-cols-3 gap-2 text-sm">
                      <div><div className="text-muted-foreground">Принято</div><div className="mt-1 font-semibold">{kg(batch.receivedKg)}</div></div>
                      <div><div className="text-muted-foreground">Вывезено примесей</div><div className="mt-1 font-semibold text-amber-800">{kg(batch.removedKg)}</div></div>
                      <div><div className="text-muted-foreground">Чистая масса</div><div className="mt-1 font-semibold text-emerald-800">{kg(batch.cleanMassKg)}</div></div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 border-t border-border pt-3 text-xs">
                      <div><span className="text-muted-foreground">Примеси</span><div className="mt-1 font-medium">{batch.impurityPercent.toLocaleString("ru-RU", { maximumFractionDigits: 3 })}%</div></div>
                      <div><span className="text-muted-foreground">Валовая урожайность</span><div className="mt-1 font-medium">{rate(batch.grossYieldTPerHa)}</div></div>
                      <div><span className="text-muted-foreground">Чистая урожайность</span><div className="mt-1 font-medium">{rate(batch.cleanYieldTPerHa)}</div></div>
                    </div>
                  </CardContent>
                </Card>
              ))}
