...
export function SystemHealthBadge({ collapsed }: { collapsed: boolean }) {
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="max-h-[86vh] max-w-3xl overflow-y-auto border-border bg-card text-foreground">
        ...
        {error ? <div className="rounded-md border border-amber-500/30 bg-amber-500/10 p-3 text-sm text-amber-800"><p>{error}</p><button type="button" className="mt-2 underline" onClick={() => void load()}>Повторить</button></div> : null}
        {payload ? <>
          <section><h3 className="mb-2 text-sm font-semibold">Система</h3><div className="grid gap-2 sm:grid-cols-2">{payload.checks.map((check) => <div key={check.key} className="flex items-start justify-between gap-3 border-b border-border py-2 text-sm"><span><b className="block font-medium">{check.label}</b>{check.detail ? <span className="block text-xs text-muted-foreground">{check.detail}</span> : null}</span><span className={check.status === "pass" ? "text-emerald-800" : check.status === "warning" ? "text-amber-800" : "text-muted-foreground"}>{check.value ?? "N/A"}</span></div>)}</div></section>
          <section><h3 className="mb-2 mt-4 text-sm font-semibold">Готовность Астык-STEM</h3><div className="grid gap-2 sm:grid-cols-2">{payload.readiness.map((item) => <div key={item.key} className="flex items-center justify-between border-b border-border py-2 text-sm"><span>{item.label}</span><span className={item.status === "ready" ? "text-emerald-800" : item.status === "missing" ? "text-red-800" : "text-amber-800"}>{item.status === "ready" ? "READY" : item.status === "missing" ? "MISSING" : "NEEDS REVIEW"} · {item.count}</span></div>)}</div>{payload.readinessSummary.blockers.length ? <p className="mt-3 text-sm text-red-800">Блокеры: {payload.readinessSummary.blockers.join(", ")}</p> : null}</section>
        </> : null}
      </DialogContent>
    </Dialog>
  );
