...
- Новый объём: `54% — общий Warm Manor интегрирован, первая браузерная матрица пройдена, все130 контуров на QA; Product rollout ещё0%`; прежние `86%` ниже относятся только к старому объёму и не являются статусом полного редизайна.
