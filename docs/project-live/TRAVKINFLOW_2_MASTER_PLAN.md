...
Обновлено: 2026-09-11 (Asia/Qyzylorda)

## Активное решение: «Контора землевладельца» — 2026-09-11

Владелец утвердил `tf2-design-reset-20260910/02-estate-register.png`, повторно приложенный как `codex-clipboard-20b4e7d0-24f3-4593-9ebe-e10c428f0231.png`. Этот выбор ЗАМЕНЯЕТ прежний Warm Manor: нейтральный каменно-бумажный рабочий лист, угольная навигация, чёткая антиква в заголовках, спокойные линии, тёмное основное действие. Светлый фон не белый/сливочный, крупные декоративные жёлтые/зелёные заливки исключены. Семантические цвета ошибок и рабочих статусов сохраняются.

Live baseline: чистая ветка `codex/travkinflow-2` / `c55c058f2c1a8da7aac53f94974b3cb65f175375`, fetch 63 ahead/0 behind origin/master и 0/0 с remote TF2. Product READY `dpl_2tR71LgNiQ8Rs3zitjPBzhRNubDr` / `4bae7d9ab2583d635d4c21b22c330ee763bd75e2`; QA READY `dpl_9y7xTmUAUcgyDZAqjDbrDCzb5MLs` / `c55c058`. Наличие C01–C33 в коде не означает завершение remote acceptance, Product волн, M09 и W6.

- [ ] R01. Перевести общие tokens/primitives/shell/portals на утверждённый нейтральный стиль; убрать локальные остатки старого оформления, не перекрашивая значения статусов.
- [ ] R02. Весовая: все 7 режимов, открытый талон/закрытие/preview/журнал/processing; единый документ и боковая очередь, inline-поиск транспорта. Не менять gross/tare/net/FIFO/ledger/idempotency/auth/driver isolation.
- [ ] R03. Структура: список→карточка→вкладки→участок→редактор→операция→материалы→диалоги. Компактные читаемые культура/площадь/сорт/репродукция, раскрываемые дополнительные параметры, сохранение всех draft fields, mix/fallow/legacy identity и delete/dirty/save guards.
- [ ] R04. Полный реестр существующих routes и вложенных состояний, включая скрытые от агронома модули. Не открывать роли или новые маршруты ради проверки дизайна.
- [ ] R05. Реальный browser gate desktop/tablet/mobile/200%zoom/keyboard/reduced-motion: поповеры, ошибки, пустые/загруженные данные, длинные RU/KZ/EN названия. Отдельно регрессионные tests критичных моделей и 7 форм весовой.
- [ ] R06. Только после gates — immutable QA, совместимый flags-off Product anchor, ранее утверждённые exact 6 DDL + 11 flags + M09, затем W6. Ни синтетических Product талонов/движений, ни blind db push. Непройденные gates не маркировать готовыми.

Rollback: до DDL можно вернуть прежний immutable Product; после DDL только проверенный TF2-compatible flags-off artifact. Изменение визуального слоя само по себе не требует DB migration. Отдельный addon гектары/калькулятор/дневные отчёты остаётся PLAN ONLY.

Ниже сохранён прежний план и доказательства; прежние проценты и цветовой контракт не описывают новую визуальную приёмку.
...
- Новый объём: `60% — Warm Manor/master-detail интегрированы, desktop dirty-guard проверен, все130 контуров на QA; Product rollout ещё0%`; прежние `86%` ниже относятся только к старому объёму и не являются статусом полного редизайна.
- Рубеж03:10UTC: independent C01-C33 source audit подтвердил все реализации, без новых изменений математики/PTC/водителя. Desktop actual ab852b9 проверил заполненную карточку, редактор, отмену перехода с сохранением черновика и явный discard. Исправлены save-in-flight race (aaa021d) и3контрастных P2 (33d4fd4); последние corrections ещё требуют immutable browser QA. Полный route inventory79 entries/13redirects; исходные33captures покрыли9настоящих экранов, не все модули/диалоги.
- Актуальный checkpoint2026-09-10: main `ebfd12f` включает shell/general/operations Warm Manor, contrast fixes, независимые контуры v3 и desktop master-detail структуры. READY Preview `dpl_67XerFbL8XrfaSHLMAot9SZTB8i5` на точном `d7bbedb5910a7c6be0155967be88036284685866`; master-detail ещё не в этом Preview.
- QA v3 migration применена ОДИН РАЗ как `20260909215439`; guarded expansion существующего import `097b2646-4adc-4842-b988-b3603889e271` выполнен:130 active /18 linked /112 unlinked. Original18 rows, source geometries/hash и поля сохранены; mismatch0. НЕ повторять expansion. Доказательства `.codex-artifacts/tf2-manor-release-20260910/qa-contours-expand-postflight.json` вне Git.
- Browser baseline exact268642f: Chromium1440/320 и WebKit390,33 route captures; application errors0, overflow0. `/tasks` redirect не считать проверкой задач; весовая проверена только до PIN gate. Для семи форм запрошен существующий QA PIN; не угадывать, не сбрасывать и не обходить. Новый v3/master-detail требует actual QA. Local critical19 suites PASS; это не Product acceptance.
- Product последний read-only baseline: health4bae7d9;341 tickets/268 batches/377 ledger, warehouse1,850,710kg, deltas0. Это внутренняя целостность, не новая сверка бумаги. Product migrations/deploy/env/business writes в новой волне0. Manifest обновляется с5 до6 точных DDL; после них rollback только на TF2-compatible flags-off artifact.
- Исторический baseline старта: main `31eab7e99cafeee03e9bbe8cb5ee6876801925cc`, clean, после fetch ahead master43/behind0, TF2 remote0/0. Product READY `dpl_2tR71LgNiQ8Rs3zitjPBzhRNubDr` / `4bae7d9ab2583d635d4c21b22c330ee763bd75e2`, health200/ok/production. QA READY `dpl_Bdu9HA7rt74Te2YGVjYLX4AxHJ11` / exact31eab7e. Product TF2 ещё не выпускался.
...
- [x] E01. Контракт разобран: старый field_id NOT NULL и DISTINCT ON(field_id) небезопасны. V3 использует самостоятельный contour_id, immutable version/CAS, nullable связь; Product остаётся на старой схеме до release gate.
- [x] E02. Реализованы source identity/name/geometry, editable display-name и nullable field-link. QA130 подтверждены:18linked/112unlinked, исходные18/геометрии/поля сохранены. Product применение относится к E06.
- [-] E03. Импорт/активация/revision/snapshot не теряют непривязанные контуры. Код и exact migration PGlite26 PASS; полный новый remote lifecycle acceptance впереди. Неизвестный matching не равен геометрической ошибке.
- [-] E04. Карта и список показывают все активные контуры. Реализованы rename/link/unlink/edit/delete/restore с audit/CAS/company guards; actual desktop unknown selection/editor/cancel PASS. Изменение и восстановление через Product не выполнялись; actual QA lifecycle и mobile проверки впереди.
- [-] E05. Local API22/PGlite26/bootstrap9 PASS; QA130/source fingerprints PASS. Полный browser rerun включает unknown/reload/role/coordinate cancellation; не считать старую карту18 или failed harness run финальной приёмкой.
...
- [-] D05. Инвентаризация79 page entries и13redirect aliases выполнена, active AST dark-style scan выполнен. Полная браузерная сверка групп/диалогов ещё идёт; dormant routes не открывать новым ролям.
