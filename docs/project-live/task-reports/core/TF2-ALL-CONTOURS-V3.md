...
Release/rollback order: keep the server write flag OFF; deploy compatible read code and apply the reviewed additive schema through ROOT's approved release path; verify v3 bootstrap/schema and fresh revision; complete role/geometry/browser checks; only then enable writes on the coordinated new client/server release. Schema-first is also safe with old reads while writes stay OFF. A stale old browser must refresh before boundary editing. If rolling back code, disable the server write flag first and retain the additive schema/history. Old code renders only field-linked geometry, so unknown contours remain stored but are not visible in its old UI; do not interpret that as data deletion.
