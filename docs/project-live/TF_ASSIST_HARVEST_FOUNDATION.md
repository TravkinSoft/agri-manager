...
## GPT-5.6 Terra default — 2026-09-14

По выбору владельца default TF Assist: direct gpt-5.6-terra, Gateway openai/gpt-5.6-terra. Gateway catalog slug перепроверен live. Явный OPENAI_ASSISTANT_MODEL сохраняет прежнюю нормализацию и приоритет; credentials не менялись. Health показывает aiDefaultModel/aiModelOverridden; внутренний smoke пишет defaultModel/modelOverridden без произвольного env-значения. Регрессия проверяет единственный вызов Terra и отсутствие fallback на 5.4-mini при HTTP403. Model smoke остаётся nonblocking, недоступный query —503/AI_UNAVAILABLE. READY не означает успешный inference.
